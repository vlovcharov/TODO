...
import { tasksApi, configApi, backupApi } from '../api';
...
export default function App() {
  ...
  const [modal, setModal] = useState(null); // { scheduledDate, level, parentId }
  ...
  const handleUpdate = useCallback((updated, allTasks) => {
    if (allTasks) { setTasks(allTasks); return; }   // full list replacement (e.g. after toggle cascade)
    if (!updated) { loadTasks(); return; }           // force reload
    setTasks(prev => {
  ...
  const handleCreated = useCallback((task) => {
    // Always reload — ensures parent subtaskIds and nested relationships are current
    loadTasks();
  }, [loadTasks]);
  ...
  const viewProps = { anchor, tasks, onUpdate: handleUpdate, onDelete: handleDelete, onCreate: handleCreate };
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-views">
          {VIEWS.map(v => (
            <button
              key={v}
              className={`sidebar-view-btn ${view === v ? 'active' : ''}`}
              onClick={() => setView(v)}
            >
              {v} View
            </button>
        ...
        <button
          ...
          onClick={() => setModal({ scheduledDate: anchor })}
        >
          <Plus size={16} />
          New Task
        </button>
        ...
        <div className="sidebar-legend">
          ...
          {Object.entries({ Daily: '#6366f1', Weekly: '#0ea5e9', Monthly: '#10b981', Yearly: '#f59e0b', LifeGoal: '#ec4899' }).map(([k, c]) => (
            <div key={k} className="legend-item">
              <div className="legend-dot" style={{ background: c }} />
              <span>{k === 'LifeGoal' ? 'Life Goal' : k}</span>
            </div>
          ))}
        </div>
